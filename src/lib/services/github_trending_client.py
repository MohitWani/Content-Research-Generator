...
class GitHubTrendingClient:
    ...
    Searches repositories and finds trending projects
    """
    ...
    async def get_trending(
        ...
        """
        Get trending repositories (approximation using search API)
        ...
        try:
            ...
            # Build query for recently created/updated repos with stars
            query = f"created:>{date_threshold} stars:>10"
            if language:
                query += f" language:{language}"
            
            # Search for AI/ML related topics
            ai_query = f"{query} (machine-learning OR deep-learning OR AI OR transformer OR LLM)"
